...
## 问题10：极限资源分配
### 问题描述：
在使用中间件的过程中，由于某些（es）对内存占用极大，导致服务器很容易oom  
经过很多次尝试，得到了以下分配：
### 解决方案：
对cpu和memory进行限制：
```text
...
###### 注：nacos太低会频繁被kill，后面会想办法解决。es太低会导致延迟特别慢，失去了使用es的本心

## 问题11：Docker部署SpringBoot项目相关问题

###  关于docker构建镜像的一系列问题

#### 打包困难
一开始要给依赖用maven工具打jar包，使用项目根目录的Install，异常报错：公共模块没有启动类  
因此我就把公共模块给移除了，但这又来了一个报错：各模块引用公共模块，但却没有公共模块的包  
如此便陷入了恶性循环
#### 解决方案：
单独先为公共模块打包  
公共模块成功后再统一打包
...
#### 镜像上传hub后服务器无法获取
由于服务器没有网络加速，在登录docker账号时便被劝退  
我的镜像没有被其他镜像源存储，导致无法拉取镜像
#### 解决方案
- 下载一个ftp工具，我这里下载的是filezilla。  
- 开放服务器22端口  
- 通过账号密码在filezilla里面连接服务器  
- 将镜像通过docker save -o E:imageId压缩到指定目录（这里最好是id）  
- 通过filezilla把压缩包发到服务器  
- 在服务器通过docker load -i name image 导入镜像  
- 然后就可以运行了
- 
#### 项目热更新无法完全实现
最开始在dockerfile的时候把端口写错了，导致端口映射不对  
后面修改了nacos里面的端口，发现容器日志内读取到了端口变更  
但是实际上端口没有更改
#### 解决方案：
重启容器

#### 服务访问响应缓慢
测试接口时，发现一个简单的查询花了接近10s才返回结果  
#### 解决方法：
通过docker stats 发现服务器内存消耗不大，但是cpu在接受请求后就居高不下（gateway和相应服务）  
后续尝试降低cpus参数看能不能解决（也可能是swap导致的缓慢）
